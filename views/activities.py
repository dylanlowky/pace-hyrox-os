...
if not activity_type:
    st.info("Select a workout type to continue.")
else:
    with st.form("quick_activity", clear_on_submit=True):
        ...
        athlete = st.selectbox("Athlete", athlete_names, disabled=not athlete_names)
        title = st.text_input("Workout name", value=title_defaults.get(activity_type, "Workout"))
        ...
        if settings.demo_mode:
            ...
            athlete_record = next(
                (
                    ...
                    if row.get("display_name") == athlete or row.get("name") == athlete
                ),
                None,
            )
            if athlete_record is None:
                ...
                PaceRepository().add_activity(
                    athlete_record["id"], workout_date, activity_type, title or activity_type,
                    distance, duration, avg_hr, rpe, pain, pain_location, notes, trained_together,
                )
...
filtered = activities if athlete_filter in {None, "All"} else activities[activities["athlete"] == athlete_filter]
