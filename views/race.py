...
st.title("Race")
st.caption("The active goal drives summaries and coaching.")

race = st.session_state.get("active_race")
if race:
    with st.container(border=True):
        st.markdown(f"### {race['name']}")
        st.write(f"{race['category']} · {race['date']:%d %B %Y}")
        st.metric("Days remaining", race["days_remaining"])
        st.write(f"Target time: **{race['target_time']}**")
        st.success("Active race")
else:
...
with st.expander("Edit active race" if race else "Create active race", expanded=not race):
    with st.form("edit_race"):
        ...
        race_date = st.date_input(
            "Race date",
            value=race["date"] if race else date(2026, 11, 22),
        ...
        try:
            if settings.demo_mode:
                st.session_state.active_race = {
                    "id": race.get("id") if race else "demo-race",
                    "name": name,
                    "date": race_date,
                    "category": category,
                ...
                PaceRepository().save_active_race(
                    st.session_state.household_id,
                    name, race_date, category, target_time,
                    race.get("id") if race else None,
                )
...
if settings.demo_mode:
    st.caption("Completed races will remain here when the next race becomes active.")
